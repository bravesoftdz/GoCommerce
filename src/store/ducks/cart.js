...
export default function cart(state = INITIAL_STATE, action) {
  switch (action.type) {
    case Types.ADD_PRODUCT: {
      ...
      if (indexItem > -1) {
        return {
          ...state,
          data: state.data.map(
            (elem, index) => (index === indexItem ? { ...elem, quantity: elem.quantity + 1 } : elem),
          ),
        };
      }
    ...
    case Types.REMOVE_PRODUCT:
      return { ...state, data: state.data.filter(item => item.id !== action.payload.data.id) };
...
export const Creators = {
  addProduct: data => ({ type: Types.ADD_PRODUCT, payload: { data } }),
  removeProduct: data => ({ type: Types.REMOVE_PRODUCT, payload: { data } }),
  setQuantity: (id, quantity) => ({ type: Types.SET_QUANTITY, payload: { id, quantity } }),
};
